...
use crate::machine::to_physical_addr;
...
buffer! {
    GetArmMemoryBuffer,
    Tag::GetArmMemory,
    base_address: MaybeUninit<u32>,
    region_size: MaybeUninit<u32>,
}
...
impl Mailbox<'_> {
    ...
    fn send<T>(&mut self, buffer: &mut T) -> bool {
        ...
        let cache_line_size = 1 << ((ctr_el0 >> 16) & 0xF);

        // Clean and invalidate all the cache lines for the buffer to make sure that the mailbox reads properly and we read the response properly
        for addr in (virtual_addr..(virtual_addr + mem::size_of::<T>())).step_by(cache_line_size) {
            unsafe {
                asm! {
                    "dc cvac, {}",
                    in(reg) addr,
        ...
        // Make sure the buffer writes are fully complete, and the cache operations complete
        ...
        // Invalidate all the cache lines for the buffer to make sure that we read the response properly
        for addr in (virtual_addr..(virtual_addr + mem::size_of::<T>())).step_by(cache_line_size) {
            unsafe {
                asm! {
                    "dc ivac, {}",
    ...
    pub fn get_arm_memory(&mut self) -> Option<(u32, u32)> {
        let mut buffer = GetArmMemoryBuffer::new(MaybeUninit::uninit(), MaybeUninit::uninit());
        buffer.base_address.write(0x123);
        buffer.region_size.write(0x456);

        if self.send(&mut buffer) {
            unsafe { core::arch::aarch64::__dmb(OSHST) }
            // SAFETY: The pointer is appropriately constructed from the buffer
            let base_addr = unsafe { ptr::addr_of!(buffer.base_address).read_volatile() };
            let size = unsafe { ptr::addr_of!(buffer.region_size).read_volatile() };

            let base_addr = unsafe { base_addr.assume_init() };
